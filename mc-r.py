    ...
    def draw_highscore_table(self):
        ...
        for i, creature in enumerate(sorted_creatures):
            ...
            self.screen.blit(text_surface, 
                           (self.width - 200, start_y + i * 25))
            
            # Draw progress bar
            required_nutrition = calculate_required_nutrition(creature.level)
            progress = creature.nutrition / required_nutrition
            bar_width = 50
            bar_height = 5
            bar_x = self.width - 200 + text_surface.get_width() + 10
...
class Creature:
    def __init__(self, x, y, level, bot_id, color):
        ...
        self.nutrition = 0
        self.hp = 100
        self.speed = 6.3  # 90% of player speed (6.3/7)
        self.base_radius = 15
